...
class _AppointmentLayoutState extends State<AppointmentLayout> {
  ...
  void _updateTimelineMonthAppointmentDetails(
    ...
    final DateTime visibleStartDate = widget.visibleDates[0];
    final DateTime visibleEndDate = widget.visibleDates[visibleDatesLength - 1];
    for (int i = 0; i < _appointmentCollection.length; i++) {
      final AppointmentView appointmentView = _appointmentCollection[i];
      if (appointmentView.canReuse || appointmentView.appointment == null) {
        continue;
      ...
      if (index == -1 && startTime.isBefore(visibleStartDate)) {
        index = 0;
      }
      ...
      int endIndex =
          DateTimeHelper.getVisibleDateIndex(widget.visibleDates, endTime);
      if (endIndex == -1 && endTime.isAfter(visibleEndDate)) {
        endIndex = visibleDatesLength - 1;
      }

      /// The width for the appointment UI, calculated based on the date index
      /// difference between the start and end time of the appointment.
      double width = (endIndex - index + 1) * cellWidth;
